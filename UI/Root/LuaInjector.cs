    ...
    public class LuaInjector : MonoBehaviour
    {
        ...
        public void InjectScene()
        {
            ...
            if (null != mainCamera)
            {
                ...
                if (cameraGo.GetComponent<CameraDrag>() != null)
                {
                    Injection cameraDragInj = new Injection()
                    {
                        key = "env.cameraDrag",
                        typeName = typeof(CameraDrag).FullName,
                        value = cameraGo.GetComponent<CameraDrag>(),
                        asComponent = true,
                    };
                    injects.Add(cameraDragInj);
                }
            }
            //BuildRoot
            var buildRoot = GameObject.Find("BuildRoot");
            if(buildRoot)
            {
                Injection buildInj = new Injection()
                {
                    key = "placeManager",
                    typeName = typeof(PlacementManager).FullName,
                    value = buildRoot.GetComponent<PlacementManager>(),
                    asComponent = true,
                };
                injects.Add(buildInj);
            }
