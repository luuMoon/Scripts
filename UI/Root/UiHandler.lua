...
function UiHandler:BindDropDown(dropDown,onDropDown,wildCard)
	local handle = self:AddHandle(onDropDown,wildCard)
	LuaUtility.BindDropDownValChanged(dropDown,handle)
end

function UiHandler:UnbindDropDown(dropDown,wildCard)
	local handle = self:RemoveHandle(wildCard)
	LuaUtility.UnBindDropDownValChanged(dropDown,handle)
end
...
function UiHandler:UnregisterDragEvents(handler)
	LuaUtility.BindDragEvents(handler)
end

--返回值是屏幕position
function UiHandler:RegisterDragEvent(handler, onBeginDrag, onDrag, onEndDrag )
	LuaUtility.BindDragEvent(handler,  bind(onBeginDrag,self),bind(onDrag,self), bind(onEndDrag,self))
end

function UiHandler:RegisterDoubleClick(handler,onDoubleClick)
	LuaUtility.BindDoubleClickEvent(handler,  bind(onDoubleClick,self))
end

function UiHandler:UnregisterDoubleClick(handler)
	LuaUtility.BindDoubleClickEvent(handler)
end
